...
function getTema() {
    ...
    if (classe.includes("alimentacao"))
        ...
        return "substancias nocivas";
}

function cookieTema(){
    cookies = document.cookie;
    temaAtual = getTema(); //preencher aqui com o tema atual
    if (cookies.search("cookieTema" + temaAtual) == -1) {
        //codigo para enviar visita tema para base de dados
        //para arranjar id cookie sessao, inicio = cookies.indexOf("cookieSessao=") + 13; cookies.substring(inicio, inicio + 7);
        inicio = cookies.indexOf("cookieSessao=") + 13;
        var idCookie = cookies.substring(inicio, inicio + 7);
        ...
        $.ajax({
            url: "/handlers/Estatistica.ashx?type=1",
            type: "POST",
            data: { tipo: "tema", id: idCookie, dispositivo: tipoDispositivo, tema: temaAtual }, //alimentacao, sexualidade ou substancias nocivas
            dataType: "json",
            success: function (result) {
                alert("ok");
            }
        ...
        document.cookie = "cookieTema" + temaAtual + "= 1; expires=" + date.toUTCString() + ";path=" + path;
    }
}
